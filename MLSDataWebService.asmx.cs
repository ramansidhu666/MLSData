...
namespace MLSWebService
{
    ...
    public class MLSDataWebService : System.Web.Services.WebService
    {
        ...
        [WebMethod]
        public DataTable GetResidentialPropertiesWithPriceSort(string MLSID_City_PostalCode, string propertyType, string MinPrice, string MaxPrice, string BedRooms, string BathRooms, string SaleLease,string SortPrice="")
        {
            DataTable dtMLS = new DataTable("MLSDATA");
            MLSData.DAL.MLSData obj = new MLSData.DAL.MLSData();
            obj.MLSID_City_PostalCode = MLSID_City_PostalCode;
            ...
            obj.BathRooms = BathRooms;
            obj.SaleLease = SaleLease;
            obj.SortPrice = SortPrice;
            return dtMLS = obj.GetResidentialProperties();
        }
        [WebMethod]
        public DataTable GetResidentialPropertiesDetail(string MLSID_City_PostalCode, string propertyType, string MinPrice, string MaxPrice, string BedRooms, string BathRooms, string SaleLease)
        {
            DataTable dtMLS = new DataTable("MLSDATA");
            MLSData.DAL.MLSData obj = new MLSData.DAL.MLSData();
            obj.MLSID_City_PostalCode = MLSID_City_PostalCode;
            ...
            obj.BathRooms = BathRooms;
            obj.SaleLease = SaleLease;
            return dtMLS = obj.GetResidentialPropertiesDetail();
        }
         [WebMethod]
        ...
        public DataTable GetImageByMLSID(string MLSID_City_PostalCode)
        {
            ...
            if (dtMLSImage.Rows[0]["VOX"].ToString().ToLower() == "false")
                ...
                sourcePath = Server.MapPath("IDXResidential/");
            else
        ...
        {
            if (String.IsNullOrEmpty(SaleLease))
            {
                SaleLease = "0";
            }
        ...
        public DataTable GetPropertiesByMLSID_Condo(string MLSID)
        {
            ...
            foreach (DataRow dr in dtMLSImage.Rows)
            {
                foreach (FileInfo files in dir.GetFiles("Photo" + dr["MLS"].ToString() + "*.*"))
                {
                    DataRow d = dtFiles.NewRow();
                    d[0] = dr["MLS"].ToString();
                    d[1] = "http://webservices.only4agents.com/IDXImagesCondo/" + files.Name;
                    dtFiles.Rows.Add(d);
                }
            }
        ...
        public string InsertVertualTourImages(String MLSID, String VimageBase64)
        {
            ...
            try
            {
                ...
                string SavePath = "http://74.208.69.145:8060/VertualImages/" + MLSID + @"/" + ImageName;
